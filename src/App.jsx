...
const fmtDate = d => { try { return new Date(d).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"}); } catch(_){ return d||""; } };
const fmtTS   = d => { try { return new Date(d).toLocaleString("en-GB",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"}); } catch(_){ return ""; } };
...
function AdminPanel({ state, setState }) {
  ...
  const delCg = id => { const{[id]:_,...rest}=state.assignments; setState({...state,caregivers:state.caregivers.filter(c=>c.id!==id),assignments:rest}); };
  ...
  const delPt = id => { const nA={}; Object.entries(state.assignments).forEach(([k,v])=>{nA[k]=v.filter(p=>p!==id);}); const{[id]:_,...rPD}=state.patientData; setState({...state,patients:state.patients.filter(p=>p.id!==id),assignments:nA,patientData:rPD}); };
